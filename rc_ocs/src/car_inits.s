...
CAR1_INIT:
	...
	move.w  #0*64/2,MOVER_X_VELOCITY_OFFSET(a0)   	 	; initial x velocity (velocity)
	move.w  #0*64/2,MOVER_Y_VELOCITY_OFFSET(a0)   	 	; initial y velocity (velocity)
	...
	move.w  #305,MOVER_STEER_DIRECTION_OFFSET(a0) 	 	; where the car should point at the beginning (degrees)? (steer_direction) (range 0-359)
	...
	move.w  #1*64/20,MOVER_BRAKE_COEFFICIENT_OFFSET(a0)	; brake coefficient, the higher the value, the strongest the brakes of the car (brake_factor)
